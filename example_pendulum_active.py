...
def get_pendulum_data(n_ics,params):
    t,x,dx,ddx,z,dz,tau = generate_pendulum_data(n_ics,params)
    ...
    data['tau'] = tau.reshape((n_ics*t.size, -1))
    #adding noise
...
def plot(n_ics,params):
    t, x, dx, ddx, X, Xdot, tau = generate_pendulum_data(n_ics,params)
    print(x.shape)
    imglist = []
    for i in range(500):
...
def generate_pendulum_data(n_ics,params):
    def pend(y,t,dc,g,l,Omega):
        theta, omega = y
        dydt = [omega, -dc*omega/1 - g*np.sin(theta)/l+np.cos(Omega*t)]
        return dydt
    ...
    while (i < n_ics):
        ...
        if np.abs(z0[1]**2/2. - np.cos(z0[0])) > .99:
            continue
        z[i] = odeint(pend, z0, t , args = (dc,g,l,Omega))
        'z.shape:(n_ics,t,2)'
        'theta,theta_dot'
        dz[i] = np.array([pend(z[i,j,:], t[j], dc, g, l, Omega) for j in range(len(t))])
