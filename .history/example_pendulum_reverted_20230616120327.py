...
def pendulum_to_movie(X,Xdot,n_ics,params,attenuation_rate):
    ...
    for i in range(X.shape[0]):
        ...
        dx[i, :, :] = (create_image(X[i, 0], X[i, 1], len)*argument_derivative(X[i,0],X[i,2],X[i,1],X[i,3],len,attenuation_rate))
        ddx[i, :, :] = create_image(X[i, 0], X[i, 1], len)*((argument_derivative(X[i,0],X[i,2],X[i,1],X[i,3],len,attenuation_rate))**2
