...
def pendulum_to_movie(X, Xdot,n_ics,params):
    ...
    create_image = lambda theta1,theta2,len : np.exp(-((y1-len*np.cos(theta1-np.pi/2))**2 + (y2-len*np.sin(theta1-np.pi/2))**2)/.05)\
                                                + 0.5*np.exp(-((y1-len*np.cos(theta2-np.pi/2))**2 + (y2-len*np.sin(theta2-np.pi/2))**2)/.05)
    ...
    argument_derivative = lambda theta1,dtheta1,theta2,dtheta2,len : -1/.05*(2*(y1 - len*np.cos(theta1-np.pi/2))*len*np.sin(theta1-np.pi/2)*dtheta1 \
                                                      ...
                                                      -0.5/.05*(2*(y1 - len*np.cos(theta2-np.pi/2))*len*np.sin(theta2-np.pi/2)*dtheta2 \
                                                      + 2*(y2 - len*np.sin(theta2-np.pi/2))*(-len*np.cos(theta2-np.pi/2))*dtheta2)
    
    argument_derivative2 = lambda theta1,dtheta1,ddtheta1,theta2,dtheta2,ddtheta2,len : 
                                                               -2/.05*((len*np.sin(theta1-np.pi/2))*len*np.sin(theta1-np.pi/2)*dtheta1**2 \
    ...
    for i in range(X.shape[0]):
        if params['changing_length'] == True:
            ...
            len = 1
    x[i,j] = create_image(X[i,0],X[i,1],len)
    dx[i,j] = (create_image(X[i,0],X[i,1],len)*argument_derivative(X[i,0],X[i,2],X[i,1],X[i,3],len))
    ddx[i,j] = create_image(z[i,j,0],len)*((argument_derivative(X[i,0],X[i,2],X[i,1],X[i,3],len))**2 \
